...
class ReservasConnection:
    ...
    def write(self, data):
        try:
            with self.conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO "reservas" (estado, fecha_reserva, forma_pago, monto, pasajero_id) 
                    VALUES (%(estado)s, %(fecha_reserva)s, %(forma_pago)s, %(monto)s, %(pasajero_id)s)
                """, data)
            self.conn.commit()
        except Exception as e:
    ...
    def update(self, data):
        try:
            with self.conn.cursor() as cur:
                cur.execute("""
                    UPDATE "reservas" 
                    SET estado = %(estado)s, fecha_reserva = %(fecha_reserva)s, forma_pago = %(forma_pago)s, monto = %(monto)s, pasajero_id = %(pasajero_id)s 
                    WHERE id = %(id)s
                """, data)
            self.conn.commit()
        except Exception as e:
